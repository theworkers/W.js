...
$(function () {
	...
	test("Display", function () {
		...
		ok(gradient.compute($(".box").length), "Computing a gradient for each div.box");
		
		var gradientArray = gradient.getGradientArray();
		ok(gradientArray && gradientArray.length > 0, "Got gradients");

		$(".box").each(function(i, el) {
			//console.log('hsl('+gradientArray[i].h+','+gradientArray[i].s+'%,'+gradientArray[i].l+'%)');
			$(el).css("background-color", 'hsl('+gradientArray[i].h+','+gradientArray[i].s+'%,'+gradientArray[i].l+'%)');
		});
