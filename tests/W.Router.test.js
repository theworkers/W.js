...
test("Trigger with additional args", 13, function () {
	var one = "one";
	var two = "two";
	var router =  new W.Router();
	ok(router, "Router was created");
	...
	var routeWithParams = router.map("/test/:one/:two/:three/").to(function (params, var1, var2) {
		ok(true, "Router fired");
		equal(params.one, "1", ":one is 1");
		equal(params.two, "fish", ":two is fish");
		equal(params.three, "pizza", ":three is pizza");
		console.log("the arguments are", arguments);
		equal(var1, one, "var1 is one");
		equal(var2, two, "var2 is two");
	});
	ok(route, "Added route/map");
	equal(typeof router.routes["/test/"], "object", "Router has route");
	equal(router.trigger("/test/", one, two), router, "Triggering route");
	equal(router.trigger("/test/1/fish/", one, two), router, "Triggering with wrong number of params and no fallback");
	equal(router.trigger("/test/1/fish/pizza/", one, two), router, "Triggering with wrong number of params and no fallback");
});
